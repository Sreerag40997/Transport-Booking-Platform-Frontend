...
export default function BusConfirmationPage() {
  ...
  const handleGetTicket = async () => {
    ...
    try {
      const ticket = await busApi.getTicket(booking.id);
      setETicket(ticket);
    } catch (err) {
  ...
  if (!booking || !busSelectedInstance) {
    return (
  ...
  const primaryPassenger = booking.passengers?.[0] || { first_name: user?.name?.split(' ')[0] || 'Traveler' };
  const departureTime = busSelectedInstance?.departure_time
    ? new Date(busSelectedInstance.departure_time).toLocaleString('en-IN', { weekday: 'short', day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })
    : 'N/A';
    ...
    <main className="pt-24 md:pt-40 pb-32 px-6 md:px-12 max-w-[1440px] mx-auto">
          ...
          <div className="flex flex-col sm:flex-row gap-4 pt-4">
            {eTicket ? (
              ...
              >
                ...
                {ticketLoading ? 'Fetching...' : 'Get Bus Ticket'}
              </button>
            )}
            <button
              onClick={() => router.push(`/buses/status/${booking.pnr}`)}
          ...
          <div className="bg-surface-container-lowest editorial-shadow p-12 lg:p-16 flex flex-col items-center justify-center space-y-6 text-center border border-outline-variant/10 relative overflow-hidden group">
            ...
            {booking.seat_class && (
              <div className="bg-secondary/10 px-6 py-2 mt-2">
                <span className="text-[9px] font-label font-black uppercase tracking-widest text-secondary">{booking.seat_class} · {booking.trip_type?.replace('_', ' ')}</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="bg-surface-container-low p-10 md:p-16 border border-outline-variant/5 editorial-shadow relative overflow-hidden">
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-12 mb-16">
            <div className="text-center md:text-left">
              <div className="font-headline text-5xl text-primary leading-none tracking-tighter mb-3">
                {booking.boarding_point?.stop_name || busSelectedInstance?.origin}
              </div>
              <div className="font-label text-[9px] uppercase tracking-[0.4em] text-outline font-black">Boarding Point</div>
            </div>
            <div className="flex-grow w-full max-w-xs relative flex items-center justify-center opacity-20">
              ...
              <span className="material-symbols-outlined absolute bg-surface-container-low px-6 text-primary">directions_bus</span>
            </div>
            <div className="text-center md:text-right">
              <div className="font-headline text-5xl text-primary leading-none tracking-tighter mb-3">
                {booking.dropping_point?.stop_name || busSelectedInstance?.destination}
              </div>
              <div className="font-label text-[9px] uppercase tracking-[0.4em] text-outline font-black">Dropping Point</div>
            </div>
          </div>

          <div className="grid grid-cols-2 md:grid-cols-4 gap-10 pt-10 border-t border-outline-variant/10">
            <div>
              <span className="font-label text-[9px] uppercase tracking-[0.3em] text-outline block mb-2 font-black">Bus Number</span>
              <span className="font-headline text-2xl text-primary uppercase">{busSelectedInstance?.bus_number}</span>
            </div>
            <div>
            ...
            <div>
              ...
              <span className="font-headline text-2xl text-primary">₹{booking.total_amount?.toLocaleString()}</span>
            </div>
            <div>
      ...
      <section>
        <div className="flex items-end justify-between border-b border-outline-variant/10 pb-6 mb-10">
          ...
          <span className="font-label text-[9px] uppercase tracking-[0.4em] text-outline font-black">{booking.passengers?.length} Traveler{booking.passengers?.length !== 1 ? 's' : ''}</span>
        </div>
