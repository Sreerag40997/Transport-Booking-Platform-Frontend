...
import { MapPin, Plane, Calendar, Clock, Loader2, SearchX, ArrowRight, CheckCircle2, Ticket } from 'lucide-react';
    ...
    return (
      <div className="min-h-screen pt-24 flex items-center justify-center bg-slate-50">
        <div className="flex flex-col items-center gap-4 text-emerald-600">
           ...
           <p className="font-bold text-slate-500 animate-pulse">Loading your dashboard...</p>
        </div>
      </div>
    );
  }

  const sortedBookings = [...bookings].sort((a, b) => new Date(b.created_at || b.CreatedAt) - new Date(a.created_at || a.CreatedAt));

  const getStatusBadge = (status) => {
    switch(status?.toUpperCase()) {
        case 'CONFIRMED': return <span className="bg-emerald-100 text-emerald-700 px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1"><CheckCircle2 size={12}/> Confirmed</span>;
        case 'PENDING': return <span className="bg-amber-100 text-amber-700 px-3 py-1 rounded-full text-xs font-bold">Pending Payment</span>;
    ...
    <div className="min-h-screen bg-slate-50 pt-24 pb-20 px-4 md:px-8">
      <div className="max-w-6xl mx-auto">
        ...
        <header className="mb-10">
            <h1 className="text-4xl font-black text-slate-800 tracking-tight">My Trips</h1>
            <p className="text-slate-500 mt-2 text-lg">Manage your upcoming flights and view past adventures.</p>
        </header>
        ...
        {sortedBookings.length === 0 && !error ? (
            ...
            >
                ...
                <h3 className="text-2xl font-bold text-slate-700 mb-2">No trips found</h3>
                <p className="text-slate-500 max-w-md mx-auto mb-8">It looks like you haven't booked any flights with us yet. Ready for your next adventure?</p>
                <Link href="/" className="bg-emerald-500 hover:bg-emerald-600 text-white font-bold py-3 px-8 rounded-full shadow-lg shadow-emerald-500/30 transition-transform active:scale-95 flex items-center gap-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {sortedBookings.map((booking, idx) => {
                    ...
                    const formattedDate = flightDate.toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' });
                    const formattedTime = flightDate.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' });
                    
                    return (
                        <Link href={`/trips/${booking.id}`} key={booking.id}>
                            <motion.div 
                            ...
                            >
                                    ...
                                    <div className="absolute inset-0 flex items-center justify-center flex-col z-10 px-6">
                                        <div className="flex w-full items-center justify-between text-slate-700 font-black text-2xl">
                                            ...
                                            <div className="flex-1 px-4 flex items-center justify-center relative">
                                                ...
                                                <Plane size={24} className="text-emerald-500 relative z-10 bg-slate-100 px-1" />
                                            </div>
                                ...
                                <div className="p-6 flex-1 flex flex-col bg-gradient-to-b from-white to-slate-50/50">
                                    <div className="flex justify-between items-start mb-6">
                                        <div>
                                            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Booking Ref</p>
                                            <p className="font-mono text-lg font-bold text-slate-700">{booking.pnr || 'PENDING'}</p>
                                        </div>
                                    ...
                                    <div className="w-full pt-6 mt-4 border-t border-slate-100 border-dashed flex justify-between items-center group-hover:border-emerald-200 transition-colors">
                                        <span className="font-bold text-sm text-slate-500 flex items-center gap-2">
                                            <Ticket size={16} /> {booking.passengers?.length || 1} Passenger(s)
                                        </span>
